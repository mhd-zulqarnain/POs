...
class AdminCustomersFragment : BaseFragment() {
    ...
    private fun initView(view: View) {
        ...
        vmCustomer = ViewModelProviders.of(activity!!,viewModelFactory).get(DashboardCustomerViewModel::class.java)
    ...
    private fun setUpDistributorRecyclerView(list: ArrayList<LocalCustomer>) {
        ...
        val currentMonthStart ="${cal.get(Calendar.MONTH) + 1}/1/$year  "
        val currentMonthEnd ="${cal.get(Calendar.MONTH) + 1}/31/$year  "

        val lastMonthStart ="${cal.get(Calendar.MONTH)}/1/$year  "
        val lastMonthEnd ="${cal.get(Calendar.MONTH)}/31/$year  "
        tvCount.setText("Count: ${filtered.size.toString()}")
        rvCustomer.layoutManager =
            LinearLayoutManager(activity!!)
        rvCustomer.adapter =
            RecyclerViewGeneralAdapter(filtered as  ArrayList<LocalCustomer>, R.layout.single_row_dasboard_detail_customer)
            { itemData, viewHolder ->
